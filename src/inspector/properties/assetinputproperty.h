...
namespace Slick {
    ...
    namespace Inspector {
        ...
        class AssetInputProperty : public Property
        {
            Q_OBJECT
            Q_PROPERTY(Core::Scene* scene READ scene WRITE setScene)

        public:
            AssetInputProperty(const QString& name, const QString& displayName, const DataSource& dataSource, Core::Scene* scene, QObject* parent = nullptr) :
