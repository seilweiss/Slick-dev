...
namespace Slick {
    ...
    FirstPersonCameraController::FirstPersonCameraController(RenderContext* context) :
        CameraController(context),
        m_moveSpeed(50.0f),
        m_rotateSpeed(glm::radians(10.0f)),
        m_pos(0.0f),
        m_yaw(0.0f),
        m_pitch(0.0f),
        m_oldVel(0.0f),
    ...
    void FirstPersonCameraController::update()
    {
        ...
        if (dyaw == 0.0f) dyaw = m_oldDYaw * 0.75f;
        if (dpitch == 0.0f) dpitch = m_oldDPitch * 0.75f;
