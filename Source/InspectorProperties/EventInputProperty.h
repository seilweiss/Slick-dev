...
namespace Slick {
    ...
    class EventInputProperty : public InspectorProperty
    {
        Q_OBJECT
        Q_PROPERTY(Scene* scene READ scene WRITE setScene)

    public:
        EventInputProperty(const QString& name, const QString& displayName, const InspectorDataSource& dataSource, Scene* scene, QObject* parent = nullptr) :
